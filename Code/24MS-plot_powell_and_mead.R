...
mostlab <- paste(month.name[month(mostrd)],year(mostrd), "Most Probable Inflow with a Lake Powell release of",
                format(powReleaseWY$MostRel[1],nsmall = 2), "in WY", powReleaseWY$WY[1], "and", 
                format(powReleaseWY$MostRel[2],nsmall = 2), "in WY", powReleaseWY$WY[2])

minlab <- paste(month.name[month(minrd)],year(minrd), ifelse(minLab_droa, 'DROA Probable Minimum', "Probable Minimum"),
                "Inflow with a Lake Powell release of",  format(powReleaseWY$MinRel[1],nsmall = 2), "in WY", 
                powReleaseWY$WY[1], "and", format(powReleaseWY$MinRel[2],nsmall = 2), "in WY", powReleaseWY$WY[2])

maxlab <- paste(month.name[month(maxrd)],year(maxrd), ifelse(maxLab_droa, 'DROA Probable Maximum', "Probable Maximum"),
                "Inflow with a Lake Powell release of",  format(powReleaseWY$MaxRel[1],nsmall = 2), "in WY", 
                powReleaseWY$WY[1], "and", format(powReleaseWY$MaxRel[2],nsmall = 2), "in WY", powReleaseWY$WY[2])
#######################################################################################
...
gg <-
  ...
  labs(y = "Pool Elevation (ft)", x = NULL, color = NULL, linetype = NULL,
       ...
       subtitle = paste('Projections from', 'April and July 2023 24-Month Study Inflow Scenarios'),
