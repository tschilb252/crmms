...
for (j in 1:length(slots)) {
  dfj = read_excel(file.path(data_dir, data_fl), sheet = slots[j], skip = 2)[,1:34] %>% na.omit()
...
cloud_model = 'Colorado River Mid-term Modeling System (CRMMS)' 
...
lab_names <- c("24-Month Study Probable Minimum", 
               "24-Month Study Probable Maximum", 
...
# highlight trace ----------------------------------------
# define the traces to highlight
trace_highlight <- 'ESP 2001' #'ESP 2011' #'ESP 2001' #c('ESP 2001', 'ESP 2011')
fname <- '2002' # what to call the file name
highlight_name <- '2002-2003 hydrology' #'2012-2013 hydrology' #'2002-2003 hydrology' #'2002-2003 and 2012-2013 hydrology'

df_24MS_p$trace_labels[df_24MS_p$Trace %in% trace_highlight] <- highlight_name
df_24MS_m$trace_labels[df_24MS_m$Trace %in% trace_highlight] <- highlight_name
custom_colors <- c(custom_colors, c('highlight' = '#ff6600'))
custom_lt <- c(custom_lt, c('highlight' = 1))
custom_size <- c(custom_size, c('highlight' = 1))
custom_alpha <- c(custom_alpha, c('highlight' = 1))
names(custom_colors)[length(custom_colors)] <- highlight_name
names(custom_lt)[length(custom_lt)] <- highlight_name
names(custom_size)[length(custom_size)] <- highlight_name
names(custom_alpha)[length(custom_alpha)] <- highlight_name



# custom storage label - maf / bcm ---------------------------
# assumes values are initially in af
us_met_storage <- function(x) {
  us <- scales::comma(x, scale = 1/1000000, accuracy = 0.01)
  met <- scales::comma(x * 1.23348183754752e-06, accuracy = 0.01) # convert AF to BCM
  paste(us, '/', met)
}
...
p_labs <- us_met_storage(CRSSIO::elevation_to_storage(p_breaks, 'powell'))
...
gg <- 
  ...
  geom_hline(yintercept = 3490, color = '#c30010', linetype = 1, size = 1.4) +
  # highlight an ESP trace
  geom_line(
    data = filter(df_24MS_p, Trace %in% trace_highlight),
  ...
  #geom_hline(yintercept = 3490, color = '#800000', linetype = 1, size = 1.2) +
...
ggsave(file.path(data_dir, paste(fname, sep = "_", "crmmsCloud_powell.png")), 
       width = 11, height = 8)

## Mead -------------------------
m_breaks <- seq(900, 1250, 25)
m_labs <- us_met_storage(CRSSIO::elevation_to_storage(m_breaks, 'mead'))
m_breaks2 <- seq(900, 1250, 5)
yy <- c(975, 1150) # NULL for default ylimit
gg <-
  ggplot(df_stat_m, aes(x = Date, fill = Cloud)) +
  scale_fill_discrete(type = cloud_color) +
  
  # highlight esp traces(s)
  geom_line(
    data = filter(df_24MS_p, Trace %in% trace_highlight),
    aes(x = Date, y = value, color = 'red', 
        alpha = trace_labels, group = Trace,
...
ggsave(file.path(data_dir, paste(fname, sep = "_", "crmmsCloud_mead.png")), 
       width = 11, height = 8)
